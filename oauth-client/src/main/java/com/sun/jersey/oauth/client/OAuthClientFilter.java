 ...
 *
 * If the URI's for requesting request and access tokens and authorization are provided, as well as the AuthHandler
 * implementation, the filter also takes care of handling the OAuth authorization flow. <p> Note: This filter signs the
 * request based on its request parameters. For this reason, you should invoke this filter after any others that modify
 * any request parameters. <p> Example 1:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p> Example 2 (handling authorization flow):
 *
 * <pre>
 ...
 *
 * @Override public void authorized(String token, String tokenSecret) { // store the access token for future use
 * storeAccessToken(token, tokenSecret); }
 *
 * @Override public String authorize(URI authorizationUri) { try { // ask user to authorize the app and enter the
 * verification code // generated by the server String verificationCode =
 * askUserToGoToAuthUriAuthorizeAndEnterVerifier(authorizationUri); return verificationCode; } catch (IOException ex) {
 * throw new RuntimeException(ex); } } } );
 *
 * // add the filter to the client client.addFilter(filter);
 *
 * // make calls to the protected resources (authorization is handled // by the filter (and the passed AuthHandler) as
 * needed, transparently WebResource resource = client.resource("http://my.service.uri/items"); String response =
...
public final class OAuthClientFilter extends ClientFilter {
     ...
     * Passing these URI's will cause the filter will automatically attempt to obtain the tokens based if it receives
     ...
     * @param handler Implementation of AuthHandler this filter calls to obtain user authorization and notify the
     * application of a new access token obtained. If null is passed, instead of invoking the handler for user
     * authorization when needed, {@link UnauthorizedRequestException} is thrown by the filter.
     */
    ...
    public ClientResponse handle(final ClientRequest request) throws ClientHandlerException {
        ...
        if (!request.getHeaders().containsKey("Authorization")) {
            ...
            try {
                throw new OAuthSignatureException(); // for now  //OAuthSignature.sign(new RequestWrapper(request, providers), p, secrets);
    ...
    /**
     * Implementation of this interface should be passed to the filter constructor to handle user authorization requests
     * and respond to obtaining a new access token (e.g. by storing it for future use).
     */
    public static interface AuthHandler {

        /**
         * Method called by the filter when an authorization of a request token is needed. Implementation should
         * redirect user to the authorization URI passed as the parameter to this method and return the verification
         * code (or null) generated by the server in a response to user authorization.
         *
         * @param authorizationUri Authorization URI the user should be redirected to.
         * @return verifier code that was generated by the server, null if the user refused to authorize the client.
         */
         ...
         * Application may want to store this for future use (to avoid the need for a new authorization next time it
         * runs).
         *
